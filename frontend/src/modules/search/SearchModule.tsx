      ...
      <div className="bg-white border-b border-slate-200/80 sticky top-0 z-50 shadow-xs py-2.5 sm:py-3.5">
        <div className="max-w-[1600px] mx-auto px-3 sm:px-6 lg:px-8 flex flex-col md:flex-row items-center gap-2.5 sm:gap-6">
          <div className="flex items-center justify-between w-full md:w-auto">
            ...
            <div className="md:hidden flex items-center gap-1 bg-slate-100 p-1 rounded-xl">
              <button 
                ...
                className={`px-3 py-1 rounded-lg text-xs font-bold flex items-center gap-1 transition-colors ${viewMode === 'list' ? 'bg-white text-[#7A0C2E] shadow-xs' : 'text-slate-500'}`}
              >
                <LayoutList size={13} /> List
              </button>
              <button 
                onClick={() => setViewMode('map')}
                className={`px-3 py-1 rounded-lg text-xs font-bold flex items-center gap-1 transition-colors ${viewMode === 'map' ? 'bg-white text-[#7A0C2E] shadow-xs' : 'text-slate-500'}`}
              >
                <Map size={13} /> Map
              </button>
            </div>
          ...
          <div className="hidden md:flex items-center gap-1 bg-slate-100 p-1 rounded-xl shrink-0">
            <button 
              onClick={() => setViewMode('list')}
              className={`px-3.5 py-1.5 rounded-lg text-xs font-bold flex items-center gap-1.5 transition-colors ${viewMode === 'list' ? 'bg-white text-[#7A0C2E] shadow-xs' : 'text-slate-500 hover:text-slate-900'}`}
            >
              <LayoutList size={15} /> List
            </button>
            <button 
              onClick={() => setViewMode('map')}
              className={`px-3.5 py-1.5 rounded-lg text-xs font-bold flex items-center gap-1.5 transition-colors ${viewMode === 'map' ? 'bg-white text-[#7A0C2E] shadow-xs' : 'text-slate-500 hover:text-slate-900'}`}
            >
              <Map size={15} /> Map
            </button>
          </div>
