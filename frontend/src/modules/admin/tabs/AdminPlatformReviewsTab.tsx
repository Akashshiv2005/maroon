...
export default function AdminPlatformReviewsTab({ onOpenSidebar }: { onOpenSidebar: () => void }) {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'approved':
        return <span className="px-3 py-1 bg-emerald-50 text-rose-700 border border-emerald-200 rounded-full text-xs font-bold flex items-center gap-1"><CheckCircle size={12} /> Approved</span>;
      case 'rejected':
                ...
                <div className="flex items-center justify-end gap-2 pt-4 border-t border-slate-100 relative z-10">
                  {review.status !== 'approved' && (
                    <button
                      ...
                      className="px-3 py-1.5 bg-emerald-50 text-rose-700 hover:bg-rose-100 font-bold text-xs rounded-xl transition-colors flex items-center gap-1"
                    >
