            ...
            return (
              <div key={key}>
                ...
                <div className="flex gap-2">
                  <input
                    type="text"
                    className="flex-1 w-full px-4 py-2.5 rounded-2xl border border-emerald-200 focus:ring-4 focus:ring-rose-500/15 focus:border-emerald-500 outline-none text-sm text-slate-800 font-semibold bg-white transition-all shadow-inner"
                    value={val}
                    onChange={e => setEditFormData((prev: any) => ({ ...prev, [key]: e.target.value }))}
                  />
                  <label className="flex items-center justify-center px-4 bg-emerald-100 hover:bg-rose-200 text-rose-700 rounded-2xl transition-colors border border-emerald-200 shadow-sm shrink-0 cursor-pointer">
                    <span className="text-xs font-bold mr-1">Upload</span>
                    <input type="file" className="hidden" onChange={(e) => {
                      if (e.target.files?.[0]) handleFileUpload(e.target.files[0], key);
                    }} />
                  ...
                  {val && (
                    <a href={getMediaUrl(val)} target="_blank" rel="noreferrer" className="flex items-center justify-center px-4 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-2xl transition-colors border border-slate-200 shadow-sm shrink-0">
                      <ExternalLink size={18} className="text-[#7A0C2E]" />
                    </a>
                  )}
                </div>
              </div>
