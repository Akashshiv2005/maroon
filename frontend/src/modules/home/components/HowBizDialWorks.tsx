  ...
  return (
    <motion.section id="how-it-works" initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: '-50px' }} transition={{ duration: 0.5 }} className="mb-10 px-4 py-16 bg-white backdrop-blur-md rounded-[3rem] border border-white/40 mx-4 lg:mx-0 overflow-hidden relative shadow-2xs">
      ...
      <div className="text-center mb-16 relative z-10">
        <p className="text-[#7A0C2E] text-[11px] font-black tracking-[0.2em] mb-4 flex items-center justify-center gap-3 uppercase">
          <span className="w-6 h-[2px] bg-[#7A0C2E]"></span> SIMPLE. FAST. EFFECTIVE <span className="w-6 h-[2px] bg-[#7A0C2E]"></span>
        </p>
        <h2 className="text-4xl md:text-[44px] font-black text-slate-900 mb-4 tracking-tight">How <span className="text-[#7A0C2E]">BizDial</span> Works</h2>
        ...
        <div className="w-24 h-1.5 bg-gradient-to-r from-rose-500 via-orange-400 to-transparent mx-auto mt-6 rounded-full flex justify-end items-center pr-0.5">
          <div className="w-1.5 h-1.5 bg-[#7A0C2E] rounded-full"></div>
        </div>
      </div>
      <div className="max-w-[1100px] mx-auto relative mt-12 mb-10">
        <div className="relative w-full h-[400px] hidden md:block">
          <svg className="absolute top-0 left-0 w-full h-[150px]" viewBox="0 0 1000 150" preserveAspectRatio="none">
            <defs>
              <linearGradient id="lineGrad" x1="0%" y1="0%" x2="100%" y2="0%">
                ...
                <stop offset="20%" stopColor="#7A0C2E" />
                <stop offset="50%" stopColor="#7A0C2E" />
                <stop offset="80%" stopColor="#7A0C2E" />
          ...
          {[
            { id: '01', title: 'Select Category', desc: 'Choose the most relevant category to list your business and reach active customers.', icon: <Building size={28} className="text-white" />, hex: '#7A0C2E', yOffset: 50 },
            { id: '02', title: 'List Business', desc: 'Create your business profile, upload details, images, and services instantly.', icon: <Plus size={28} className="text-white" />, hex: '#7A0C2E', yOffset: 90 },
            { id: '03', title: 'SEO Engine', desc: 'BizDial automatically generates custom SEO tags, titles, and descriptions.', icon: <TrendingUp size={28} className="text-white" />, hex: '#7A0C2E', yOffset: 40 },
            { id: '04', title: 'Rank Top', desc: 'Our ranking engine places your listing at the top of local search results.', icon: <Award size={28} className="text-white" />, hex: '#7A0C2E', yOffset: 80 },
            { id: '05', title: 'Get Leads', desc: 'Receive direct calls, messages, and quality leads from active local buyers.', icon: <Phone size={28} className="text-white" />, hex: '#7A0C2E', yOffset: 50 },
          ].map((step, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 30, scale: 0.95 }} whileInView={{ opacity: 1, y: 0, scale: 1 }} viewport={{ once: true, margin: '-50px' }} transition={{ delay: 0.2 + i * 0.15, duration: 0.35, type: "spring", stiffness: 300, damping: 25 }} className="absolute top-0 w-1/5 h-full flex flex-col items-center" style={{ left: `${i * 20}%` }}>
              ...
              <div className="absolute z-30" style={{ top: `${step.yOffset + 5}px` }}>
                ...
                >
                  <div className="w-full h-full rounded-full flex items-center justify-center bg-[#7A0C2E]">{step.icon}</div>
                </motion.div>
              </div>
              <motion.div 
        ...
        <div className="flex flex-col gap-12 md:hidden w-full relative pt-10">
          <div className="absolute left-1/2 -translate-x-1/2 top-0 bottom-0 w-1 bg-gradient-to-b from-violet-400 via-purple-400 to-[#7A0C2E] rounded-full z-0 opacity-20"></div>
          {[
            { id: '01', title: 'Select Category', desc: 'Choose the most relevant category to list your business and reach active customers.', icon: <Building size={28} className="text-white" />, hex: '#7A0C2E' },
            { id: '02', title: 'List Business', desc: 'Create your business profile, upload details, images, and services instantly.', icon: <Plus size={28} className="text-white" />, hex: '#7A0C2E' },
            { id: '03', title: 'SEO Engine', desc: 'BizDial automatically generates custom SEO tags, titles, and descriptions.', icon: <TrendingUp size={28} className="text-white" />, hex: '#7A0C2E' },
            { id: '04', title: 'Rank Top', desc: 'Our ranking engine places your listing at the top of local search results.', icon: <Award size={28} className="text-white" />, hex: '#7A0C2E' },
            { id: '05', title: 'Get Leads', desc: 'Receive direct calls, messages, and quality leads from active local buyers.', icon: <Phone size={28} className="text-white" />, hex: '#7A0C2E' },
          ].map((step, i) => (
