  ...
  return (
    ...
    >
      ...
      <div className="flex-1 w-full relative z-10">
        <motion.div initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: 0.05, duration: 0.3 }} className="inline-flex items-center gap-2 px-3.5 py-1 bg-[#7A0C2E] text-white rounded-full text-xs font-bold mb-3 sm:mb-5 shadow-md shadow-[#7A0C2E]/20">
        ...
        <motion.h2 initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.08, duration: 0.3 }} className="text-2xl sm:text-4xl md:text-5xl font-black mb-2 sm:mb-4 leading-tight text-slate-900">
          ...
          <span className="text-[#7A0C2E]">with BizDial</span>
        </motion.h2>
        ...
        <div className="grid grid-cols-2 gap-2.5 sm:gap-4 mb-6 sm:mb-8">
          {[
            { icon: <MapPin size={16}/>, label: 'High Visibility', sub: 'Rank higher in search', color: 'text-[#7A0C2E]', bg: 'bg-violet-50' },
            { icon: <TrendingUp size={16}/>, label: 'Quality Leads', sub: 'Real buyers, real calls', color: 'text-[#7A0C2E]', bg: 'bg-violet-50' },
            { icon: <BarChart size={16}/>, label: 'Business Analytics', sub: 'Track views & leads', color: 'text-[#7A0C2E]', bg: 'bg-violet-50' },
            { icon: <Star size={16}/>, label: 'Trusted Platform', sub: 'Verified by 10M+ users', color: 'text-[#7A0C2E]', bg: 'bg-violet-50' },
          ].map((f, i) => (
            <motion.div 
              key={i} 
          ...
          <div className="bg-white/80 backdrop-blur-xl rounded-3xl shadow-2xl border border-white p-6">
            ...
            <div className="grid grid-cols-3 gap-2 mt-2">
              {[
                ...
                { label: 'Leads', value: '3.2K', color: 'text-rose-600' },
