...
const getCategoryTheme = (slug?: string) => {
  const themes: Record<string, { bg: string; iconBg: string; text: string }> = {
    ...
    finance: { bg: 'from-violet-50 to-white', iconBg: 'bg-violet-100', text: 'group-hover:text-[#7A0C2E]' },
...
const Home = ({ initialData }: { initialData?: any }) => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#FFF5F2] via-white to-[#FFF9F7] font-sans text-slate-800 overflow-x-hidden">
      <motion.div 
        className="fixed top-0 left-0 right-0 h-[3.5px] bg-gradient-to-r from-[#7A0C2E] via-purple-500 to-violet-500 z-50 origin-left"
