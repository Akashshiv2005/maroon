...
const Login = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4 relative">
      <Link href="/" className="absolute top-6 left-6 sm:top-8 sm:left-8 flex items-center gap-2 text-slate-600 hover:text-[#7A0C2E] transition-colors font-bold bg-white px-4 py-2 rounded-full shadow-xs border border-slate-200">
        <ArrowLeft size={18} />
        ...
        <div className="p-8">
          <div className="text-center mb-8">
            <Link href="/" className="inline-block mb-6">
              <h1 className="text-3xl font-black tracking-tight text-slate-900">
                <span className="text-slate-900 font-extrabold">Biz</span><span className="text-[#7A0C2E] font-black">Dial</span>
              </h1>
            </Link>
          ...
          {view === 'login' ? (
            <>
              <div className="flex bg-slate-100 p-1 rounded-xl mb-8">
                <button 
                  ...
                  className={`flex-1 py-2 text-sm font-bold rounded-lg transition-all ${role === 'owner' ? 'bg-white text-[#7A0C2E] shadow-xs' : 'text-slate-500 hover:text-slate-700'}`}
                >
                  Business Owner
                </button>
                <button 
                  ...
                  className={`flex-1 py-2 text-sm font-bold rounded-lg transition-all ${role === 'admin' ? 'bg-white text-[#7A0C2E] shadow-xs' : 'text-slate-500 hover:text-slate-700'}`}
                >
                  Super Admin
                </button>
              </div>
              ...
              <form onSubmit={handleLogin} className="space-y-5">
                ...
                <div>
                  <div className="flex items-center justify-between mb-1.5">
                    ...
                    {role === 'owner' && (
                      <button type="button" onClick={() => setView('forgot_password')} className="text-sm font-semibold text-[#7A0C2E] hover:underline">Forgot password?</button>
                    )}
                  </div>
                  <div className="relative">
                    <input 
                      ...
                      className="w-full px-4 py-3 pr-12 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-rose-500 focus:border-transparent transition-all"
                      placeholder="••••••••"
                    />
                    <button
                      type="button"
                ...
                <button 
                  type="submit" 
                  className="w-full bg-[#7A0C2E] hover:bg-[#5C0520] text-white font-bold py-3.5 rounded-xl transition-colors shadow-lg shadow-[#7A0C2E]/25 mt-4"
                >
              ...
              {role === 'owner' && (
                <div className="mt-8 text-center text-sm font-medium text-slate-600">
                  Don't have an account? <Link href="/register" className="text-[#7A0C2E] font-bold hover:underline">Register your business</Link>
                </div>
              )}
            </>
          ) : (
            <form onSubmit={handleForgotPassword} className="space-y-5">
              ...
              {resetMessage && <p className="text-sm text-rose-600 font-medium text-center">{resetMessage}</p>}
